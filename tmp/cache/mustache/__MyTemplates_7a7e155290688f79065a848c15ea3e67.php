...
class __MyTemplates_7a7e155290688f79065a848c15ea3e67 extends Mustache_Template
{
...
';
        ...
        $buffer .= $this->section8ecac1e007c6393eb7efa1cae1e6b420($context, $indent, $value);
        $buffer .= $indent . '      </select>
';
...
';
    ...
    private function section8ecac1e007c6393eb7efa1cae1e6b420(Mustache_Context $context, $indent, $value)
    {
        $buffer = '';
    
        if (is_object($value) && is_callable($value)) {
            $source = '
        <option value="{{ location_id }}" {{ selected }}>{{ city }} - {{ address }} {{ postal_code }}</option>
        ';
            $result = (string) call_user_func($value, $source, $this->lambdaHelper);
            if (strpos($result, '{{') === false) {
                $buffer .= $result;
