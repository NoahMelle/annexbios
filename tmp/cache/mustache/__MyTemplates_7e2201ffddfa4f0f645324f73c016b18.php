...
class __MyTemplates_7e2201ffddfa4f0f645324f73c016b18 extends Mustache_Template
{
...
';
        ...
        $buffer .= $this->sectionCd28c688e9f2d9a4fc2466f927a6e3a9($context, $indent, $value);
        $buffer .= $indent . '      </select>
';
...
';
    ...
    private function sectionCd28c688e9f2d9a4fc2466f927a6e3a9(Mustache_Context $context, $indent, $value)
    {
        $buffer = '';
    
        if (is_object($value) && is_callable($value)) {
            $source = '
        <option value="{{ location_id }}" {{ selected }}>{{ title }}</option>
        ';
            $result = (string) call_user_func($value, $source, $this->lambdaHelper);
            if (strpos($result, '{{') === false) {
                $buffer .= $result;
